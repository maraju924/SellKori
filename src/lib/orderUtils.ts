...
export function getRiskFlags(order: Order, allOrders: Order[]): OrderRiskFlag[] {
  ...
  const identitySiblings = allOrders.filter(
    o => o.id !== order.id && orderIdentityKey(o) === orderIdentityKey(order)
  );
  if (identitySiblings.length >= 1 && !flags.some(f => f.id === 'dup')) {
    flags.push({
      id: 'identity-dup',
      label: `ডুপ্লিকেট ×${identitySiblings.length + 1}`,
      tone: 'warning',
    });
  }
